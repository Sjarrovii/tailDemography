...
def validate(x, sort_criteria=['species', 'toes', 'sex','initialCaptureDate','smallest_svl'], 
    ...
    not_validated_count = x.loc[~((x.year_diff <= 7) & (x.svl_diff >= -2)), :].shape[0]
    ...
    on {} and {} rows which do not pass validation.".format(x.liznumber.max(), sort_criteria, validation, not_validated_count))
