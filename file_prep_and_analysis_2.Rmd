...
#FIX THIS
```
add 2010 date data back into the dataset
```{r}
# data2010<-read.xlsx("./excelFiles/xCC2010x.xlsx",1)
# names(data2010)<-tolower(names(data2010))
# test<-merge(mycombfile,data2010$date, by="year")
# 
# mycombfile[which(mycombfile$year==2010),"date"]
# mycombfile$date<-asDate(mycombfile$date)
```

# Export mapped dataframe
```{r}
filename<-paste("mapped data","_",format(Sys.Date(),"%y-%m-%d"),".csv")
